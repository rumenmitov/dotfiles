...
The following snippet forces the minibuffer into a separate frame. You can then set a rule in your window-manager to control where on the screen that frame will be displayed.
...
*** COMMENT Yank-Pop and Fido-Vertical
For completions in the minibuffer frame, we use =fido-vertical-mode=. However, =fido-vertical-mode= does not play well with =yank-pop=, so we disable it for that scenario.
...
The macro =tempo/setup-mode= will create the following for the specified mode:

- a tags-list to contain the mode-specific snippet definitions
- a definition function to define mode-specific snippets
- a completion function for the mode
...
Defined the snippet definitions separately, so that I can re-use them for different modes.
...
Use =offlineimap= to sync mail locally, then open it with Gnus. To search through the mail, the external =notmuch= program should be installed on the system.
...
Fetch new mail from the mail server:
